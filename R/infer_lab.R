...
infer_lab <- function(labels, dimtype) {
    fun <- switch(EXPR = dimtype,
                  ...
                  destination = ,
                  parent = ,
                  child = infer_lab_attribute,
                  pool = infer_lab_pool,
                  triangles = infer_lab_triangles,
                  quantiles = infer_lab_quantiles,
...
infer_lab_categories <- function(labels) {
    ## check for blanks
    demcheck::err_not_blank_vector(x = labels,
                                      name = "labels")
...
infer_lab_durations_months <- function(labels) {
    ...
    p_open_last <- "^([0-9]+)m\\+$"
    ## check for blanks
    demcheck::err_not_blank_vector(x = labels, name = "labels")
    ## only need to process one instance of each label
    labels <- unique(labels)
    ## sorting should work if labels have correct format
    labels <- sort_durations(labels)
